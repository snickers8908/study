...
# How long a transfer may make NO measurable progress before we call it stalled.
# As long as the file keeps growing, the copy runs indefinitely (up to the
# hard cap inside wait_for_copy). Bump this if your slowest links pause longer.
STALL_SECS = 180
...
class CopyAborted(Exception):
    """Raised when the copy dialog didn't reach an expected prompt."""
    pass
...
def parse_free_bytes(listing: str):
    """
    Return free bytes on the filesystem from a `dir` listing, or None.
    IOS-XE: '... bytes total (NNN bytes free)'
    NX-OS:  '... bytes free' / 'Usage ...' — handled by the total-free line.
    """
    m = re.search(r"(\d[\d,]*)\s+bytes\s+free", listing, re.IGNORECASE)
    if m:
...
# One pull lock per device IP. A pull worker must hold its device's lock for
# the whole copy; a second worker targeting the same IP will fail to acquire
# it and refuse, guaranteeing at most one active copy per device.
_DEVICE_LOCKS = {}
_DEVICE_LOCKS_GUARD = threading.Lock()


def device_lock(host):
    with _DEVICE_LOCKS_GUARD:
        lk = _DEVICE_LOCKS.get(host)
        if lk is None:
            lk = threading.Lock()
            _DEVICE_LOCKS[host] = lk
        return lk
    ...
    try:
        while True:
            ...
            try:
                ...
                # Only update if still copying — never resurrect a terminal state.
                with PROGRESS_LOCK:
                    cur_state = PROGRESS.get(host, {}).get("state", "copying")
                    if cur_state not in ("done", "failed"):
                        PROGRESS.setdefault(host, {"cur": 0, "total": 0, "state": "copying"})
                        PROGRESS[host].update(cur=cur, total=total_hint, state="copying")
            except Exception:
                pass
...
# Copy completion markers seen at the very end of a successful transfer.
# Kept strict so mid-transfer SCP debug chatter doesn't look like completion.
COPY_DONE_RE = re.compile(
    r"bytes copied in [\d.]+ secs|"      # IOS-XE: "N bytes copied in X secs (...)"
    r"Copy complete|"                    # NX-OS
    r"copied.*\d+ bytes/sec",            # some NX-OS builds
    re.IGNORECASE,
)


def wait_for_copy(conn, host, image_file, fs, total_hint,
                  stall_secs=180, poll_secs=5, hard_cap_secs=7200):
    """
    Wait for an in-flight `copy scp:` to finish WITHOUT failing just because it
    is slow. The rule is: keep waiting as long as progress is being made.

    Progress = either new bytes on the session channel, or the destination
    file growing on the device (polled via `dir`). We only give up if NOTHING
    has advanced for `stall_secs` straight. `hard_cap_secs` is a final safety
    net for a truly wedged session (default 2h).

    Returns the accumulated channel output. Raises CopyAborted on stall,
    on a device-reported copy error, or on the hard cap.
    """
    buf = ""
    ...
    last_buf_len = 0
    last_poll = 0.0
    size_reached_at = None    # when the file first hit expected size

    while True:
        now = time.time()

        # 1) Drain whatever the device has emitted (non-blocking-ish).
        try:
            chunk = conn.read_channel()
        except Exception as e:
            log.debug("%s: read_channel error: %s", host, e)
            chunk = ""
        if chunk:
            buf += chunk

        # 2) Any new channel bytes count as progress.
        if len(buf) > last_buf_len:
            last_buf_len = len(buf)
            last_progress = now

        # 3) Device-side error -> stop now (don't wait out the stall timer).
        err = detect_copy_error(buf)
        if err:
            raise CopyAborted(f"device reported copy error: {err}")

        # 4) Completion marker -> done.
        if COPY_DONE_RE.search(buf):
            log.debug("%s: copy completion marker seen", host)
            return buf

        # 5) Poll the growing file size as a second progress signal. This is
        #    what makes a quiet-but-still-transferring session count as alive.
        if now - last_poll >= poll_secs:
            last_poll = now
            size = poll_file_size(conn, host, image_file, fs)
            if size is not None:
                set_progress(host, cur=size, total=total_hint, state="copying")
                if size > last_size:
                    last_size = size
                    last_progress = now
                    log.debug("%s: transfer progress %s", host, human(size))
                # File reached expected size and held for one full poll interval:
                # treat as complete even if the text marker was missed.
                if total_hint and last_size >= total_hint:
                    if size_reached_at is None:
                        size_reached_at = now
                    elif now - size_reached_at >= poll_secs:
                        log.debug("%s: file reached expected size (%s), "
                                  "treating as complete", host, human(last_size))
                        return buf

        # 6) Stall / hard-cap checks.
        if now - last_progress > stall_secs:
            raise CopyAborted(
                f"no progress for {stall_secs}s (last size "
                f"{human(last_size) if last_size >= 0 else 'unknown'}) — treating as stalled"
            )
        if now - start > hard_cap_secs:
            raise CopyAborted(f"exceeded hard cap of {hard_cap_secs}s")

        time.sleep(1)


def poll_file_size(conn, host, image_file, fs):
    """
    Best-effort file-size read on the SAME session while a copy runs.
    Cisco can't run `dir` mid-copy on the same VTY, so this will usually
    return None during transfer; the dedicated monitor thread (separate
    session) is the primary size source. Kept for completeness/robustness.
    """
    with PROGRESS_LOCK:
        p = PROGRESS.get(host, {})
        cur = p.get("cur", 0)
...
def pull_image(host, username, password, platform, vrf, image_file, server_ip,
    ...
    # Hard guard: only one pull may run per device at a time. If another worker
    # already holds this device's lock, refuse rather than open a second
    # competing copy session to the same box.
    lock = device_lock(host)
    if not lock.acquire(blocking=False):
        log.error("%s: a pull is already in progress for this device — refusing duplicate", host)
        return (host, False, "duplicate pull skipped (already in progress for this device)")
    try:
        return _pull_image_locked(host, username, password, platform, vrf,
                                  image_file, server_ip, total_hint)
    finally:
        lock.release()


def _pull_image_locked(host, username, password, platform, vrf, image_file,
                       server_ip, total_hint=0):
    dev = {
        "device_type": "cisco_nxos" if platform == "nxos" else "cisco_ios",
        "host": host,
        "username": username,
    ...
    try:
        ...
        def expect(out, pattern, step):
            """Abort the copy if the device didn't show the expected prompt."""
            if not re.search(pattern, out, re.IGNORECASE):
                raise CopyAborted(
                    f"expected '{pattern}' at step '{step}' but got: "
                    f"{out.strip().splitlines()[-1] if out.strip() else '<empty>'}"
                )

        if platform == "nxos":
            # NX-OS: VRF is an interactive prompt; source syntax uses the file.
            out = dbg_send(conn, host, "copy step: init",
                           "copy scp: bootflash:", timing=True)
            expect(out, r"source file|filename", "init->source-file")
            out += dbg_send(conn, host, "copy step: source file", image_file, timing=True)
            expect(out, r"vrf", "source-file->vrf")
            out += dbg_send(conn, host, "copy step: vrf",
                            vrf if vrf else "default", timing=True)
            expect(out, r"hostname|host name|server", "vrf->hostname")
            out += dbg_send(conn, host, "copy step: scp hostname", server_ip, timing=True)
            expect(out, r"user", "hostname->username")
            out += dbg_send(conn, host, "copy step: username", username, timing=True)
            if re.search(r"fingerprint|continue connecting|yes/no|known hosts", out, re.IGNORECASE):
                log.debug("%s: accepting host fingerprint", host)
                out += dbg_send(conn, host, "copy step: fingerprint", "yes", timing=True)
            expect(out, r"password", "username->password")
            # Send password without blocking on a fixed timeout, then wait for
            # the transfer using progress-based stall detection.
            log.debug("%s: sending scp password (masked), entering progress wait", host)
            conn.write_channel(password + "\n")
            device_log(host, "copy step: password  CMD='********'", "<sent, awaiting transfer>")
            out += wait_for_copy(conn, host, image_file, fs, total_hint,
                                 stall_secs=STALL_SECS)
        else:
            # IOS-XE: VRF must be inline on the command, NOT an interactive prompt.
            init_cmd = f"copy scp: {fs}"
            if vrf:
                init_cmd += f" vrf {vrf}"
            out = dbg_send(conn, host, "copy step: init", init_cmd, timing=True)
            expect(out, r"remote host|host name|Address", "init->remote-host")
            out += dbg_send(conn, host, "copy step: remote host", server_ip, timing=True)
            expect(out, r"source username|user", "remote-host->username")
            out += dbg_send(conn, host, "copy step: source username", username, timing=True)
            expect(out, r"source filename|filename", "username->source-file")
            out += dbg_send(conn, host, "copy step: source filename", image_file, timing=True)
            expect(out, r"destination filename|filename", "source-file->dest-file")
            out += dbg_send(conn, host, "copy step: dest filename", image_file, timing=True)
            if re.search(r"fingerprint|continue connecting|yes/no|known hosts", out, re.IGNORECASE):
                log.debug("%s: accepting host fingerprint", host)
                out += dbg_send(conn, host, "copy step: fingerprint", "yes", timing=True)
            # Password prompt must appear; if it didn't (timeout/error), abort
            # BEFORE sending the password so it can't land at the exec prompt.
            expect(out, r"[Pp]assword", "dest-file->password")
            log.debug("%s: sending scp password (masked), entering progress wait", host)
            conn.write_channel(password + "\n")
            device_log(host, "copy step: password  CMD='********'", "<sent, awaiting transfer>")
            out += wait_for_copy(conn, host, image_file, fs, total_hint,
    ...
    except CopyAborted as e:
        set_progress(host, state="failed")
        log.error("%s: copy aborted — %s", host, e)
        device_log(host, "COPY ABORTED", str(e))
        # Break out of the half-open copy dialog so we never type the password
        # (or anything else) at the exec prompt. Ctrl-C cancels the copy prompt.
        try:
        ...
        try:
            ...
            pass
        return (host, False, f"copy aborted: {e}")
    except Exception as e:
        set_progress(host, state="failed")
...
def main():
    ...
    with open(hosts_path) as f:
        raw_hosts = [ln.strip() for ln in f if ln.strip()]
    if not raw_hosts:
        log.error("No IPs in hosts file")
        print("No IP addresses found in hosts file.")
        sys.exit(1)
    # Dedupe (preserve first-seen order) so a device listed twice is never
    # assessed or pulled by more than one worker.
    hosts = list(dict.fromkeys(raw_hosts))
    dupes = len(raw_hosts) - len(hosts)
    if dupes:
        log.warning("Removed %d duplicate host entr%s from the list",
                    dupes, "y" if dupes == 1 else "ies")
        print(f"Note: ignored {dupes} duplicate host entr"
              f"{'y' if dupes == 1 else 'ies'} in the file.")
    log.info("Loaded %d unique host(s): %s", len(hosts), ", ".join(hosts))
    ...
    for host in hosts:
        ...
        # Free-space pre-check: notate and skip if the image won't fit.
        free = parse_free_bytes(listing)
        need = IMAGE_SIZE_HINT.get(image_file, 0)
        log.debug("%s: free=%s need=%s", host, free, need)
        if free is not None and need and free < need:
            msg = (f"insufficient free space on {fs}: "
                   f"{human(free)} free, need ~{human(need)}")
            notes.append(f"{host} ({model}): {msg} — skipped")
            log.error("%s: %s", host, msg)
            print(f"{host}: {msg} — skipping pull")
            conn.disconnect()
            continue
    ...
    results = []
    # Final safety: submit at most one pull future per unique device IP.
    seen_hosts = set()
    plan_unique = []
    for p in plan:
        if p["host"] in seen_hosts:
            log.warning("%s: duplicate plan entry dropped before pull", p["host"])
            continue
        seen_hosts.add(p["host"])
        plan_unique.append(p)

    with ThreadPoolExecutor(max_workers=min(len(plan_unique), 20)) as ex:
        futs = {
